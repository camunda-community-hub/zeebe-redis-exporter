...
public class ExporterDeleteAfterAcknowledgeTest {
  ...
  public ZeebeTestContainer zeebeContainer =
      ZeebeTestContainer.withCleanupCycleInSeconds(2).doDeleteAfterAcknowledge(true);
  ...
  public void shouldNotDeleteWhenConsumedButNotAcknowledged() throws Exception {
    ...
    var messages =
        redisConnection
            .sync()
            .xreadgroup(
                Consumer.from("application_2", "consumer_2"),
                XReadArgs.Builder.block(6000),
                XReadArgs.StreamOffset.lastConsumed("zeebe:DEPLOYMENT"));
    assertThat(messages.size()).isGreaterThan(0);
    var xlen = redisConnection.sync().xlen("zeebe:DEPLOYMENT");
    ...
    await()
        ...
        .untilAsserted(
            () ->
                assertThat(redisConnection.sync().xlen("zeebe:DEPLOYMENT"))
                    .isGreaterThanOrEqualTo(xlen));
  }
  ...
  public void shouldNotDeleteWhenNotAcknowledgedByAllGroups() throws Exception {
    ...
    var messages =
        redisConnection
            .sync()
            .xreadgroup(
                Consumer.from("application_3", "consumer_3"),
                XReadArgs.Builder.block(6000),
                XReadArgs.StreamOffset.lastConsumed("zeebe:DEPLOYMENT"));
    assertThat(messages.size()).isGreaterThan(0);
    for (StreamMessage<String, byte[]> message : messages) {
    ...
    messages =
        redisConnection
            .sync()
            .xreadgroup(
                Consumer.from("application_4", "consumer_4"),
                XReadArgs.Builder.block(6000),
                XReadArgs.StreamOffset.lastConsumed("zeebe:DEPLOYMENT"));
    assertThat(messages.size()).isGreaterThan(0);
