...
public class RedisSender {
    ...
    public RedisSender(ExporterConfiguration configuration, Controller controller, StatefulRedisConnection<String, ?> redisConnection, Logger logger) {
        ...
        this.redisConnection.addListener(new RedisConnectionStateListener() {
            @Override
        ...
        try {
            ...
            while (nextEvent != null) {
                for (int i = 0; i < batchSize; i++) {
                    ...
                    futures.add(commands.xadd(eventValue.stream, String.valueOf(eventValue.key), eventValue.value));
                ...
                if (futures.size() > 0) {
                    ...
                    boolean result = LettuceFutures.awaitAll(7, TimeUnit.SECONDS,
            ...
            logger.error("Error when sending events to Redis due to possible Redis unavailability: {}", ex.getMessage());
        } catch (Exception ex) {
    ...
    private boolean sendDeQueue() {
        ...
        try {
            ...
            logger.error("Error when sending dequeued events to Redis due to possible Redis unavailability: {}", ex.getMessage());
        } catch (Exception ex) {
