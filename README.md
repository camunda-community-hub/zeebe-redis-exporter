...
```
docker pull ghcr.io/camunda-community-hub/zeebe-with-redis-exporter:8.1.9-0.9.4
```
...
1. Download the latest [exporter JAR](https://github.com/camunda-community-hub/zeebe-redis-exporter/releases) (_zeebe-redis-exporter-0.9.4-jar-with-dependencies.jar_)

1. Copy the exporter JAR  into the broker folder `~/zeebe-broker-%{VERSION}/exporters`.

    ```
    cp exporter/target/zeebe-redis-exporter-0.9.4-jar-with-dependencies.jar ~/zeebe-broker-%{VERSION}/exporters/
    ```
      ...
      broker:  
        exporters:
          redis:
            ...
            jarPath: exporters/zeebe-redis-exporter-0.9.4-jar-with-dependencies.jar
    ```
...
services:
  zeebe:
    ...
    ports:
      ...
      - "9600:9600"
    volumes:
      - ../exporter/target/zeebe-redis-exporter-0.9.4-jar-with-dependencies.jar:/usr/local/zeebe/exporters/zeebe-redis-exporter.jar
